...
export function registerDeviceWorkflow(context: vscode.ExtensionContext): void {
  ...
  status.text = "$(debug-disconnect) Playdate device: unchecked"; status.tooltip = "Check the physical Playdate USB connection"; status.command = "gopdsdk.checkDeviceConnection"; status.show();
  const run = async (command: DeviceCommand, resource?: vscode.Uri): Promise<DeviceResult | undefined> => {
    ...
    try {
      ...
      if (result.failure) {
        if (command === "probe connection" && result.failure.category === "not-connected") status.text = "$(debug-disconnect) Playdate device: disconnected";
  ...
  const check = async (resource?: vscode.Uri): Promise<void> => {
    const result = await run("probe connection", resource); if (!result || result.failure) return;
    status.text = "$(device-mobile) Playdate device: connected"; status.tooltip = "USB connection verified by gopdsdk probe connection";
  ...
  const workflow = async (command: "build device" | "run device", resource?: vscode.Uri): Promise<void> => {
    if (command === "run device") {
      const connection = await run("probe connection", resource); if (!connection || connection.failure) return;
      status.text = "$(device-mobile) Playdate device: connected"; status.tooltip = "USB connection verified by gopdsdk probe connection";
    }
  ...
  context.subscriptions.push(status, vscode.workspace.registerTextDocumentContentProvider("gopdsdk-log", logs),
