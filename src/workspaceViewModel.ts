...
export function workspaceNodes(snapshot: WorkspaceSnapshot): WorkspaceNode[] {
  ...
  return [
    ...
    { label: "Project Health", description: snapshot.health, icon: snapshot.health === "toolchain installed" ? "pass" : "warning", command: "gopdsdk.showProjectHealth", group: "summary" },
    { label: "Device", description: snapshot.device, icon: snapshot.device === "connected" ? "device-mobile" : snapshot.device === "checking" ? "loading~spin" : snapshot.device === "error" ? "error" : "debug-disconnect", command: "gopdsdk.checkDeviceConnection", group: "summary" },
